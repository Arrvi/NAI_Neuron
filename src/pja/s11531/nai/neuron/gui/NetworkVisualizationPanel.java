...
public class NetworkVisualizationPanel extends JComponent {
    ...
    private       int        outputResolution = 300;
    private       double     maxOpacity       = 0.9;
    
    private EnumSet<DrawOption> drawOptions = EnumSet.of(
            DrawOption.DRAW_AXIS,
...
//                () -> {
                    ...
                    for ( int y = 0; y < outputResolution; y++ ) {
                        outputExecutor.execute( new RowGenerator( y, neuronColor ) );
                    }
//                } );
    }
    
    private class RowGenerator  implements Runnable {
        int row;
        Color[] neuronColor;
    
        public RowGenerator ( int row, Color[] neuronColor ) {
            this.row = row;
        ...
        @Override
        public void run () {
            System.out.printf( "Generating row %d of %d%n", row, outputResolution );
            for ( int x = 0; x < outputResolution; x++ ) {
                BigDecimal[] output = network.calculate(
                        new BigDecimal[] {
                                widthToUnits( x, outputResolution ),
                                heightToUnits( row, outputResolution )
                        } );
                for ( int i = 0; i < output.length; i++ ) {
                    int alpha = (int) ( output[i].doubleValue() * maxOpacity * 0xff );
                    networkOutputBuffer[i].setRGB( x, row, neuronColor[i].getRGB() & 0xffffff + ( alpha << 24 ) );
                }
            }
            repaint();
        }
